...
- **Implementado:** Ejercicios predeterminados (auto-asegurar 60), ABM ejercicios/series/rutinas, grupos musculares como entidad, asignación rutina→alumno, enlace público por token, hoja `/rutinas/hoja/{token}` (permitAll), Copiar enlace y WhatsApp desde ficha, calendario semanal con puntos verde/rojo/gris y API de asistencia, columna Presente en Mis Alumnos, ausentes on-demand al abrir calendario, modal de progreso unificado, alumno inactivo (acciones deshabilitadas), filtros alumnos persistentes, orden de series y ejercicios.
